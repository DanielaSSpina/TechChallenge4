...
st.set_page_config(page_title="Análise e Previsão do Preço do Petróleo", layout='wide')

# Caminho dos arquivos do modelo e dos dados
...
excel_path = os.path.join(base_dir, 'dataset', 'Petroleo.xlsx')  # Caminho para o arquivo de dados
...
try:
    df, data_scaled, _ = load_and_process_data(excel_path, date_cutoff=pd.to_datetime('2020-05-03'))
except Exception as e:
    st.error(f"Erro ao carregar dados: {e}")
    st.stop()

# Data inicial e limite de previsão
data_base = date(2024, 5, 20)  # Definir com a data mais recente** ajustar esse aqui
dias_limite = 15

# Entrada do usuário para a previsão
st.header("Previsão de Preço do Petróleo Brent")
st.info(f"Selecione uma data até *{dias_limite} dias* após {data_base} para prever o preço.")

data_escolhida = st.date_input(
    "Escolha uma data de previsão",
    min_value=data_base + timedelta(days=1),
    max_value=data_base + timedelta(days=dias_limite),
    value=data_base + timedelta(days=1),
)

dias_para_previsao = (data_escolhida - data_base).days
seq_len = 10

if st.button('Prever Preço'):
    with st.spinner('Calculando...'):
        try:
            previsao = predict(dias_para_previsao, data_scaled, seq_len)
            if previsao is None:
                st.error("Erro durante a previsão.")
                st.stop()
            
            datas_previsao = pd.date_range(start=df['data'].iloc[-1], periods=dias_para_previsao+1, freq='D')[1:]
            
            # Separando dados para teste e calculando métricas
            tam_treino = int(len(data_scaled) * 0.8)
            X_teste, y_teste = create_sequences(data_scaled[tam_treino:], seq_len)
            r2, mse, mae, mape, rmse = evaluate_lstm_model(model, X_teste, y_teste, scaler)
            
            # Texto com as métricas
            metrica_texto = f"R²: {r2:.5f} | MSE: {mse:.5f} | MAE: {mae:.5f} | MAPE: {mape:.5f} | RMSE: {rmse:.5f}"
            
            # Plotando dados históricos e previsão
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df['data'], y=df['preco'], mode='lines', name='Histórico'))
            fig.add_trace(go.Scatter(x=datas_previsao, y=previsao.flatten(), mode='lines', name='Previsão'))
            fig.update_layout(
                title="Previsão do Preço do Petróleo Brent",
                xaxis_title="Data",
                yaxis_title="Preço do Petróleo (US$)",
                annotations=[{
                    'text': metrica_texto,
                    'showarrow': False,
                    'xref': 'paper',
            ...
            # Exibindo previsões em tabela
            previsao_df = pd.DataFrame({
                "Data": datas_previsao.strftime("%Y-%m-%d"),
                "Preço Estimado": previsao.flatten().round(2)
            })
            st.subheader("Previsões por Data")
            st.write(previsao_df.set_index("Data"))
